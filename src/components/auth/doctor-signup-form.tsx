...
export function DoctorSignup({
  ...
  const doctorExistsByPhone = async (phone: string): Promise<boolean> => {
    const data = await fetcher<ExistsResponse>(
      "doctor",
      `/doctor/exists?contact=${encodeURIComponent(phone)}`
    );
  ...
  useEffect(() => {
    ...
    if (status === "thank-you" || via === "google") {
  ...
  const handleOtpRequest = async (data: OtpValues) => {
    setOtpSending(true);
    try {
      // check first using axios helper
      const exists = await doctorExistsByPhone(data.contact);
      if (exists) {
        toastAndNavigate(dispatch, true, "error", "Phone already registered.");
        setStep("otpRequest"); // stay on step 1
        return;
      }

      // only send OTP if not exists
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const result = await signIn("google", {
        redirect: false,
        callbackUrl: `/signup-as-doctor?status=thank-you&via=google`,
      });

      // If NextAuth gives us a URL back, handle outcomes here
      if (result?.url) {
        if (result.url.includes("error=DoctorAlreadyExists")) {
          toastAndNavigate(
            ...
            true,
            "error",
            "This email is registered. Sign in or try another Google account."
          );
          return;
        }
        if (result.url.includes("status=thank-you")) {
          // Your existing target thank-you state
          router.push("/signup-as-doctor?status=thank-you&via=google");
          return;
        }
      ...
      // Fallback: keep your original redirect behavior (so your code isn't impacted)
      await signIn("google", {
        callbackUrl: `/signup-as-doctor?status=thank-you&via=google`,
      });
    } catch (error) {
